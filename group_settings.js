document.addEventListener("DOMContentLoaded", function () {
  ...
  function createGroupForm() {
    ...
    formContainer.addEventListener("click", function (e) {
      if (e.target.classList.contains("remove-url-btn")) {
        ...
        if (
          ...
          groupsContainer.children[0] !== formContainer
        ) {
    ...
    const customGroupNameInput =
      formContainer.querySelector(".custom-group-name");
    groupNameSelect.addEventListener("change", function () {
      customGroupNameInput.style.display =
        groupNameSelect.value === "other" ? "block" : "none";
    });
  ...
  saveBtn.addEventListener("click", function () {
    ...
    document.querySelectorAll(".form-container").forEach((formContainer) => {
      ...
      const urls = Array.from(form.querySelectorAll('input[name="url"]')).map(
        (input) => {
...
`;
        ...
        const deleteButtons = savedGroupsContainer.querySelectorAll('.delete-btn');
  ...
  function deleteGroup(groupName) {
    if (confirm(`Are you sure you want to delete the group "${groupName}"?`)) {
      chrome.runtime.sendMessage(
        { action: "deleteGroup", groupName: groupName },
        (response) => {
          if (response.status === "success") {
            console.log("Group deleted successfully");
            renderSavedGroups(); // Re-render the groups after deletion
          } else {
